#!/bin/bash
# Behavioral cloning pre-training: Q_LEARNING observes MINIMAX vs MINIMAX games
# and learns from both players' moves. Run this on a fresh model (after backup_model.sh
# + deleting the old model + restarting the server) to seed the network with
# MINIMAX-level common sense before switching to self-play RL (train_q_learning.sh).
#
# Usage: ./pretrain_minimax.sh [batches] [games_per_batch]
...
echo "Behavioral cloning: MINIMAX vs MINIMAX, Q_LEARNING observing"
echo "Batches: $BATCHES x $GAMES games"
  ...
  "players": [
    {
      ...
      "color": "BLUE",
      "strategy": "MINIMAX",
      "params": { "learn": true, "trainingTarget": "TERMINAL_OUTCOME", "depth": 1 }
    },
    {
      "playerType": "COMPUTER",
      "color": "RED",
...
echo "Pre-training complete. Now run train_q_learning.sh to refine with self-play RL."
